...
class ParticleFilter(object):
    ...
    def __init__(self, num_particles, sampling_method, x_range, y_range, means,
                 stds, motion_std_err, measurement_std_err, num_predict_steps,
                 plot_particles):
        ...
        self._init_particles(sampling_method, x_range, y_range, means, stds)
    ...
    def _init_particles(self, sampling_method, x_range, y_range, means, stds):
        ...
        if sampling_method == 'uniform':
            ...
            states = torch.normal(
                means.expand(self._num_particles, -1),
                stds.expand(self._num_particles, -1))
        else:
            raise ValueError(
                "Argument 'sampling_method' must be 'uniform' or 'gaussian'")

        # create particles with equal weights
        weights = torch.FloatTensor((1 / self._num_particles, ))
        self._particles = torch.cat(
            (states, weights.expand(self._num_particles, -1)), dim=1)

    def _motion_update(self, particles, stds, dt):
        noisy_speed = particles[:, 2:4] + torch.randn(self._num_particles,
                                                      2) * stds[0]
        noisy_acc = particles[:, 4:6] + torch.randn(self._num_particles,
                                                    2) * stds[1]
        noisy_acc_acc = particles[:, 6:8] + torch.randn(
            self._num_particles, 2) * stds[2]
    ...
    def _measurement_update(self, pt, std):
        ...
        self._particles[:, -1] = torch.FloatTensor(
            scipy.stats.norm(0, std).pdf(dist + torch.randn(1) * std))
        self._particles[:, -1] += 1.e-300  # avoid round-off to zero
        self._particles[:, -1] /= sum(self._particles[:, -1])  # normalize

    def _systematic_resample(self):
        positions = (torch.rand(1) + torch.arange(
            self._num_particles).float()) / self._num_particles
    ...
    def _resample_from_index(self, indexes):
        ...
        self._particles[:, -1].new_full((self._num_particles, 1),
                                        1.0 / self._num_particles)

    def _estimate(self, particles):
        return np.average(
            particles[:, :2].numpy(), weights=particles[:, -1].numpy(), axis=0)
    ...
    def update_state(self, pt):
        ...
        self._particles = self._motion_update(self._particles,
                                              self._motion_std_err, 1)
